...
export default function VolunteerRequestDetails() {
  ...
  useEffect(() => {
    const fetchRequest = async () => {
      try {
        setLoading(true);
        const response = await api.get(`/volunteer-requests/${id}?populate=applications`);
                      ...
                      {applications.map(app => (
                        <li key={app._id} className="py-3 flex justify-between items-center">
                          <div>
                            ...
                            {app.portfolioLink && (
                              <a href={app.portfolioLink} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline">Portfolio</a>
                            )}
                            <Badge variant={app.status === 'accepted' ? 'default' : app.status === 'rejected' ? 'destructive' : 'secondary'} className="ml-2">{app.status}</Badge>
                          </div>
                          {app.status === 'pending' && (
                            <div className="flex gap-2">
                              <Button variant="outline" onClick={() => handleApplicationAction(app._id, 'reject')} disabled={isHandlingApplication}>Reject</Button>
                              <Button onClick={() => handleApplicationAction(app._id, 'accept')} disabled={isHandlingApplication}>Accept</Button>
                            </div>
                          )}
                        </li>
                      ))}
